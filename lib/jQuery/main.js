...
function cartFill(){
  ...
  for (var i = 0; i < items.length; i++) {
    const coffee = document.querySelectorAll('.coffeeTitle').innerHTML;
    const pic = document.querySelectorAll('#coffee1 #coffeePic1').this.attr('src');
    const des = document.querySelectorAll('.coffeeDesc1').text;
    const cnt = document.querySelectorAll('.qty').value;
    items[i].addEventListener('click', function() {
      if(items[i].cnt > 0) {
        document.getElementById('cartProduct').html(coffee[i]);
        document.getElementById('cartPic').html('<div class="col-sm-2 hidden-xs" id="cartPic"><img src='+pic[i]+' alt="..." class="cart-img"/></div>');
        document.getElementById('cartDesc').html(des[i]);
        document.getElementById('cartQty').val(cnt[i]);
        return false;
      } 
    })
  }

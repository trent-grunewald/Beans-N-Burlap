...
const qtyRequest = document.querySelectorAll('.qty');
const sumDisplay = document.getElementById('itemCount');
const add = document.querySelectorAll('.addItem');

for (let index = 0; index <qtyRequest.length; index++) {
  qtyRequest[index].addEventListener('change', function(e) {
    let sum =0;
    qtyRequest.forEach(function(){
      let amount = this.value;
      if (!isNaN(amount)) sum += amount;
    })
    for(let test = 0; test < add.length; test++) {

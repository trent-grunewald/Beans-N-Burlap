...
document.querySelector('.menuType').addEventListener('change', function(){
  ...
  for (let index = 0; index < menu.length; index++) {
    //Grabs the index of the array object
      const element = menu[index];
      //html update target
      let meal = document.querySelector('.entree');
      //HTML option creation
      let food = document.createElement("option");
      //verifying menu type / name (Breakfast or Lunch)
      if (element.name === menuType) {
        //adds the value and innerHTML to the newly created Options
        food.value = food.entree;
        food.text = food.entree;
        //Adds the food variable, no before needed = null
        meal.add(food, null);
      }
    }
    // for (let index = 0; index <= menu.length; index++) {
    ...
    //     if(menuType === menu.name) {;
    //         opt.value = element.entree;
    //         opt.text = element.entree;
    //         meal.add(opt, null);
    //   }
    // }
});
